...
class MetaLearner_exp(object):
        ...
        for epoch in range(self.noptepochs):
            ...
            for start in range(0, nbatch, nbatch_train):
                ...
                #for name, param in self.policy.named_parameters():
                    #if torch.sum(torch.gt(param.grad,self.grad_clip)):
                        #print('before clipping',torch.sum(torch.gt(param.grad,self.grad_clip)))
                grad_norm.append(torch.nn.utils.clip_grad_norm_(self.policy.parameters(), self.grad_clip))
                for name, param in self.policy.named_parameters():
                    if torch.sum(torch.gt(param.grad,self.grad_clip)):
                        print('after clipping',torch.sum(torch.gt(param.grad,self.grad_clip)))
                        pdb.set_trace()
                optimizer.step()
    ...
    def inner_loss_ppo_noUpdate(self, episodes, first_order,params=None, ent_coef=0,
                        vf_coef=0, nenvs = 1):
        """Compute the inner loss for the one-step gradient update. The inner
        loss is PPO with clipped ratio = new_pi/old_pi.
        Can make cliprange adaptable.
    ...
    def surrogate_loss_ppo(self, episodes, old_pis=None):
        losses, kls, pis = [], [], []
        if old_pis is None:
            old_pis = [None] * len(episodes)
        ...
        for train_episodes, valid_episodes in episodes:
            params,grad_norm = self.adapt_ppo(train_episodes)
            loss = self.inner_loss_ppo_noUpdate(valid_episodes,first_order=True)
            self.logger.info("in surrogate_loss_ppo")
            losses.append(loss)
    ...
    def step_ppo(self, episodes, max_kl=1e-3, cg_iters=10, cg_damping=1e-2,
             ls_max_steps=10, ls_backtrack_ratio=0.5):

        #calculate batch losses
        batch_losses = self.surrogate_loss_ppo(episodes)

        #setup optimizer and optimize
        optimizer = optim.SGD(self.policy.parameters(), lr=self.ppo_lr, momentum=self.ppo_momentum)
        optimizer.zero_grad()   # zero the gradient buffers
